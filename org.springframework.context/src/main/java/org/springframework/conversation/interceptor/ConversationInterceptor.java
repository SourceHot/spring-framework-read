...
public class ConversationInterceptor implements MethodInterceptor {
	 ...
	 *
	 * @see org.springframework.conversation.manager.ConversationManagerImpl
	 */
		...
		try {
			...
			if (conversationAttribute != null && conversationAttribute.shouldStartConversation()) {
				Conversation conversation =
						conversationManager.beginConversation(false, conversationAttribute.getJoinMode());
			...
			if (conversationAttribute != null && conversationAttribute.shouldEndConversation()) {
				conversationManager.endCurrentConversation(conversationAttribute.getConversationEndingType());
			}
			...
			if (conversationAttribute != null) {
				conversationManager.endCurrentConversation(
						ConversationEndingType.getFailureType(conversationAttribute.getConversationEndingType()));
			}
			throw th;
		}
